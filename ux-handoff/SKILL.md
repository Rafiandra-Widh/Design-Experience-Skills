...
description: >
  ...
  ("anotasi", "redline", "catatan interaksi"); audit a frame's readiness before
  handoff — naming, auto-layout, token coverage, states ("cek kesiapan", "audit frame");
  or audit implemented UI against the Figma design — pixel-perfect QA ("cek implementasi
  vs figma", "pixel perfect", "audit hasil coding", "design QA", "kok beda sama desain").
  Modes: spec, component-doc, annotate, readiness, qa.
  Not for implementing the design as code (use figma-design-to-code) or reviewing
  visual quality (use ux-design critique).
user-invocable: true
argument-hint: "[spec|component-doc|annotate|readiness|qa]"
---

# ux-handoff

Everything between "the design is done" and "it ships correctly": specs, component docs, redlines, readiness audits, and post-implementation pixel-perfect QA.
...
| `qa` | Compare implemented UI (live page/screenshot/code) against the Figma design | `reference/qa.md` |

## Cross-mode rules

1. **Real values only.** Every number and color comes from the source of truth: `get_design_context` for structure/dimensions, `get_variable_defs` for tokens, `get_metadata` for node inventory. In `qa` mode the implementation side is data too — computed styles or source code, never visual guesses; screenshot-only comparisons are always labeled as estimates. NEVER estimate values from a screenshot — if data can't be fetched, say which values are missing rather than guessing.
